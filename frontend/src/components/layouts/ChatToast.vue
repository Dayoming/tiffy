<template>
    <div class="toast-container toast-chat d-flex gap-3 align-items-end">
        <div
            :class="['toast toast-chat show', isMobile ? 'mobile-toast-message' : '']"
            role="alert"
            aria-live="assertive"
            aria-atomic="true"
            :style="isMobile ? '' : style"
        >
            <div class="toast-header" :style="{ display: isMobile ? 'none' : '' }">
                <img
                    src="/icon/profile-default-icon.png"
            ...
            <div
                :class="[
                    'chat-conversation-content custom-scrollbar',
                    isMobile ? 'mobile-toast-height' : '',
                ]"
                ref="chatContents"
            >
                <div
                    ...
                    :key="message.id"
                    class="d-flex"
                    :class="message.sender === 'me' ? 'justify-content-end text-end mb-1' : 'mb-1'"
                >
                    <div class="w-100">
                        <div
                            class="d-flex flex-column"
                            :class="message.sender === 'me' ? 'align-items-end' : 'align-items-start'"
                        >
                            <div
                                :class="message.sender === 'me' ? 'bg-primary text-white' : 'bg-light text-secondary'"
                                class="p-2 px-3 rounded-2"
                            >
                                {{ message.content }}
                            </div>
